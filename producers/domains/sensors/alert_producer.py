...
def run(config: dict):
    ...
    topic = config["topic"]
    sleep_seconds = config.get("sleep_seconds", 3)  # Medium frequency streaming
    
    logger.info("Starting sensor alert streaming producer with Avro + Schema Registry")
    ...
    while True:
        ...
        event = wrap_event(
            domain="sensors",
            event_type="sensor_alert_triggered",
            payload=payload,
        )
        
        key = payload.get("alert_id") or event["event_id"]
