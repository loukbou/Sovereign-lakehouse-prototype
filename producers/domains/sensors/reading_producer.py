...
from domains.sensors.generator import generate_reading
...
def run(config: dict):
    ...
    topic = config["topic"]
    sleep_seconds = config.get("sleep_seconds", 1)  # High frequency streaming
    
    logger.info("Starting sensor reading streaming producer with Avro + Schema Registry")
    ...
    while True:
        ...
        event = wrap_event(
            domain="sensors",
            event_type="sensor_reading_recorded",
            payload=payload,
        )
        
        key = payload.get("reading_id") or event["event_id"]
