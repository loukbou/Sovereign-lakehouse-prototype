...
from domains.logistics.generator import generate_vehicle
...
def run(config: dict):
    ...
    topic = config["topic"]
    sleep_seconds = config.get("sleep_seconds", 5)  # Batch/reference, slower rate
    
    logger.info("Starting logistics vehicle batch/reference producer with Avro + Schema Registry")
    logger.info("Publishing to topic: %s", topic)
    
    error_topic = config["producer_error_topic"]
    
    while True:
        payload = generate_vehicle()
        ...
        key = payload.get("vehicle_id") or event["event_id"]
