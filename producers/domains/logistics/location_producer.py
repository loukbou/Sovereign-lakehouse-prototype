...
from domains.logistics.generator import generate_location
...
def run(config: dict):
    ...
    topic = config["topic"]
    sleep_seconds = config.get("sleep_seconds", 10)  # Batch/reference, even slower rate
    
    logger.info("Starting logistics location batch/reference producer with Avro + Schema Registry")
    ...
    while True:
        ...
        key = payload.get("location_id") or event["event_id"]
